...
namespace QuarterbackRating.Views
{
    ...
    public sealed partial class Create : Page
    {
        ...
        private void CreateSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            var quarterbackWithRating = repo.ReadAll().LastOrDefault(q => q.Name == Name.Text);

            var message = new MessageDialog(quarterbackWithRating.Name + "'s QB Rating is: " + quarterbackWithRating.Rating);
            ShowPopUp(message);

            this.Frame.Navigate(typeof(MainPage));
        }

        static async void ShowPopUp(MessageDialog message)
        {
